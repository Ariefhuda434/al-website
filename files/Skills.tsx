    ...
    <Section id="skills">
      ...
      <Stagger className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3" gap={0.06}>
        {list.map((s, i) => {
          ...
          // Kartu terakhir melebar kalau jumlahnya ganjil, supaya grid tidak menggantung.
          const stretch = i === list.length - 1 && list.length % 3 === 1;

          return (
            <StaggerItem key={`${s.id}-${i}`} className={stretch ? "lg:col-span-1" : ""}>
              <motion.article
                whileHover={{ y: -6 }}
              ...
              >
                ...
                {s.en ? <p className="mt-2 text-berry">{s.en}</p> : null}
                {s.idn ? <p className="mt-1 text-sm italic text-muted">{s.idn}</p> : null}

                {(s.id === "handmade" || s.id === "flower") && (
                  <FlowerHead
                    tone="blush"
                    petals={8}
